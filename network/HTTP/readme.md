...
第一次是由客户端发起，客户端会向服务端发送一个报文，在报文里面，SIN 位标志为 1，服务端收到这个报文之后就知道，客户端要像我发起一个新的连接，
于是服务端就向客户端发送一个确认消息包，在这个消息包里面，ACK 位置 1，以上两次连接之后，其实客户端已经知道了：我既能和服务端发送消息，又可以
...
- 客户端发送 SYN,表明要向服务器建立连接，同事带上序列号 ISN
- 服务器返回 ACK（序列号为客户端序列号+1）作为确认。同事发送 SYN 作为应答（SYN 的序列号为服务端唯一的序号）
